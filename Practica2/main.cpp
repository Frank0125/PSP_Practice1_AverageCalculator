...
int main() {
    try {
        ...
        if(n > 15 || n < 5) {
            throw invalid_argument("El tamaño de lista de números a calificar no debe ser menor a " +
            std::string("5 o mayor a 15."));
        }

        float *list = new float[n];
        for (int i = 0; i < n; i++) {
            cout << "Ingresa un número para el calculo: " << endl;
            cin >> list[i];
            if (cin.fail()) {
                throw invalid_argument("El tipo de valor introducido no es aceptado.");
            }
            if(list[i] > 100 || list[i] < 5) {
                throw invalid_argument("El valor del número a calcular no debe ser menor a 5 o mayor a" +
                std::string(" 100"));
            }
        }
